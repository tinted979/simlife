...
// build_conway_lut constructs the lookup table for Conway's Game of Life.
// This procedure runs at compile time via the #run directive.
@(private)
...
//
// Returns: Next state (0 = dead, 1 = alive)
@(private)
apply_conway_rules :: proc(neighbor_count: int, current_state: int) -> u8 {
	if current_state == CELL_ALIVE {
		...
		return CELL_DEAD // Dies from isolation or overcrowding
	} else {
...
//
// This encoding allows us to pack both inputs into a single index:
...
step :: proc(state: ^State) #no_bounds_check {
	...
	for y in 1 ..= state.grid_height {
		...
		for x in 1 ..= state.grid_width {
			...
			// Look up the next state using our compile-time LUT
...
//
// Ghost cell layout:
//   T T T T T   (T = top ghost row)
//   L . . . R   (L = left ghost, R = right ghost, . = visible cell)
//   L . . . R
//   L . . . R
//   B B B B B   (B = bottom ghost row)
@(private)
